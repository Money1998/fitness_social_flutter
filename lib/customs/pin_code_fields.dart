...
// selects the right fill color for the field
  ...
  List<Widget> _generateFields() {
    ...
    for (int i = 0; i < widget.length; i++) {
      result.add(
        Padding(
          padding: new EdgeInsets.only(left:i==0 ? 0.0 : 16.0),
          child: AnimatedContainer(
            ...
            child: Center(
              child: AnimatedSwitcher(
                ...
                child: Text(
                  widget.obsecureText && _inputList[i].isEmpty
                      ? "*"
                      : _inputList[i],
                  key: ValueKey(_inputList[i]),
                  style: widget.obsecureText && _inputList[i].isEmpty ? TextStyle(
                      color: Colors.black,fontSize: 20.0,fontWeight: FontWeight.bold) : widget.textStyle,
                ),
              ),
            ),
          ),
